...
from exp.RQ4.GAN.helper import get_dimwise_prob_metrics, save_current_plot, score_real_fake, make_dimwise_probability_plot, \
...
class BaseGAN():
    ...
    def _pretrain_aux_teacher(self, X, y):
        ...
        X = X[y == self.minority_label]
        cv_preds = cv_preds[y == self.minority_label]
        # make target
    ...
    def _compute_aux_teacher_loss(self, fake, y_batch):
        if not self.condition:
            ...
            fake_X = fake[y_batch.view(-1) == self.minority_label]
    ...
    def resample(self, X, y):
        ...
        print(y.sum(), (1-y).sum())
        n_minority_needed = int(abs(((1 - y).sum() - y.sum())))

        if self.netG.condition:
            # create the desired number of minority cases directly
            X_fake = self.sample(n_minority_needed, y=self.minority_label)
            ...
            n_synth_needed = int(1.2 * abs(((1 - y).sum() - y.sum())) / synth_imb_ratio)
