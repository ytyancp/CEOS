...
def RW(name, par, explainer=None, smote=False, small=0.05, act=False, number=5):
    ...
    for j in range(0, len(files) - 2):
        df1 = prepare_data(files[j])
        df2 = prepare_data(files[j + 1])
        for i in range(1, 21):
            col1 = df1.iloc[:, i]
            col2 = df2.iloc[:, i]
...
def ceos_ar(name, number_act=10):
    start_time = time.time()
    files = [name[0], name[1], name[2]]
    deltas = []
    score = []
    ...
    ceos = CEOS()
    X_resampled, y_resampled = ceos.fit_resample(train, defects)
    ...
    [support_before, support_after, confidence],
    [confidence_before, confidence_after, lift],
    lift_value
    ]
    '''
    ...
    formatted_rules.to_csv('../../{}_rules.csv'.format(name[0].split('-')[0]), index=False)
