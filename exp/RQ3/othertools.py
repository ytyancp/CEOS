...
def get_index(name):
    feature = ['cbm', 'lcom3', 'rfc', 'max_cc', 'cbo', 'moa', 'avg_cc', 'noc', 'ce', 'npm', 'ca', 'mfa', 'lcom', 'amc',
               'cam', 'dam', 'ic', 'wmc', 'loc', 'dit']
...
def flip(data_row, local_exp, ind, cols, n_feature=5, actionable=None):
    counter = 0
    cache = []
    trans = []
    ...
    record = [0 for _ in range(20)]
    tem = data_row.copy()
    result = [[0 for _ in range(2)] for _ in range(20)]
...
def hedge(arr1, arr2):
    ...
    denominator = n1 + n2 - 1 - 1
    sp = (numerator / denominator) ** .5
    delta = np.abs(m1 - m2) / sp
    c = 1 - 3 / (4 * (denominator) - 1)
    return delta * c
...
def frequent_set(name):
    df1 = prepare_data(name[0])
    df2 = prepare_data(name[1])
    df3 = prepare_data(name[2])
    df11 = df1.iloc[:, 1:]
    df22 = df2.iloc[:, 1:]
    df33 = df3.iloc[:, 1:]
